...
async def fetch_item_details(api_base, course_id, item, headers, current_path="", userid="", progress_callback=None):
    ...
    try:
        ...
        # FIRST: Try encrypted_links[] - contains direct signed URL (path) + base64 AES key
        # Screenshot confirmed: path=signed URL, key=base64("2837273") - no Vercel needed!
        import base64 as _b64
        encrypted_links_data = data.get("encrypted_links", [])
        encrypted_link_found = False

        if encrypted_links_data:
            for link in encrypted_links_data:
                epath = link.get("path", "")
                ...
                if epath and epath.startswith("http") and "appx.co.in" in epath:
                    ...
                    if ekey_b64:
                        try:
                            ekey_val = _b64.b64decode(ekey_b64).decode('utf-8').strip()
                        except Exception:
                            try:
                                ekey_val = decode_base64(ekey_b64) or ""
                            except Exception:
                                ekey_val = ""

                    if not ekey_val and 'encrypted-' in epath:
                        import re as _re
                        em = _re.search(r'encrypted-([a-fA-F0-9]+)', epath)
                        if em:
                            ekey_val = em.group(1)

                    if ekey_val:
                        outputs.append(f"{prefix}{vt} : {epath}*{ekey_val}*{course_id}*{fi}")
                    else:
