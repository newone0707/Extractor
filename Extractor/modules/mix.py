...
from Extractor.modules.custom_crypto import encrypt_appx_params, encrypt_direct_url
import re
...
async def fetch_item_details(api_base, course_id, item, headers, current_path="", userid="", progress_callback=None):
    try:
        ...
        token = headers.get("token") or headers.get("Authorization") or ""
        ...
        if item_link:
            ...
            if '.pdf' in item_link.lower() or item.get('pdf_link'):
                pdf_key = ""
                ...
                if encrypted_links:
                    ...
                    if k:
                        ...
                        if key_val:
                            pdf_key = key_val
                if not pdf_key and 'encrypted-' in item_link:
                    m = re.search(r'encrypted-([a-fA-F0-9]+)', item_link)
                    if m:
                        pdf_key = m.group(1)
                enc_pdf = encrypt_direct_url(item_link, pdf_key if pdf_key else None)
                fallback_outputs.append(f"{prefix}{vt} PDF : {enc_pdf}")
            else:
                enc_payload = encrypt_appx_params(api_base, course_id, fi, token, userid)
                fallback_outputs.append(f"{prefix}{vt} : {enc_payload}")
        ...
        if fl:
            ...
            if dfl:
                ...
                if '.m3u8' in dfl or '.mp4' in dfl or 'genomic' in dfl or '/' in dfl:
                    enc_payload = encrypt_appx_params(api_base, course_id, fi, token, userid)
                    outputs.append(f"{prefix}{vt} : {enc_payload}")
                else:
        ...
        for pdf_num in range(1, 3):
            ...
            if pdf_link:
                ...
                if dp:
                    dpk = None
                    if pdf_key:
                        dpk_dec = decrypt(pdf_key)
                        if dpk_dec and dpk_dec != "abcdefg":
                            dpk = dpk_dec
                    
                    enc_pdf = encrypt_direct_url(dp, dpk)
                    outputs.append(f"{prefix}{vt} PDF{pdf_num if pdf_num > 1 else ''} : {enc_pdf}")
        ...
        for topic in j4.get("data", []):
            ...
            for video in j5.get("data", []):
                ...
                if vl:
                    ...
                    if video_link:
                        video_link = re.sub(r'\.zip', '.m3u8', video_link, flags=re.IGNORECASE)
                    vi = video.get("id")
                    enc_payload = encrypt_appx_params(api_base, raw_text2, vi, token, userid)
                    all_outputs.append(f"({subject_title}) {vt}:{enc_payload}")
                else:
                    vi = video.get("id")
                    ...
                    if "material_type" in j6["data"]:
                        ...
                        if mt == "VIDEO" or mt == "PDF":
                            ...
                            if p1:
                                ...
                                dp1k = p1key if (p1key and p1key != "abcdefg") else None
                                enc_pdf1 = encrypt_direct_url(dp1, dp1k)
                                all_outputs.append(f"({subject_title}) {vt} PDF:{enc_pdf1}")
                            if p2:
                                dp2 = decrypt(p2.split(":")[0])
                                pp2 = j6["data"].get("pdf_encryption_key", "")
                                pkey = pp2.split(":")[0] if pp2 else ""
                                p2key = decrypt(pkey) if pkey else ""
                                dp2k = p2key if (p2key and p2key != "abcdefg") else None
                                enc_pdf2 = encrypt_direct_url(dp2, dp2k)
                                all_outputs.append(f"({subject_title}) {vt} PDF-2:{enc_pdf2}")
